...
suite('Converter Service Tests', () => {
    ...
    test("Test Find Markers For Document - no scores", () => {
        //JSON response
        const jsonResponse = '[{"documentUri":"refactor-document","repository":"test-repo","line":193,"content":"tole je nova vsebina","isDeleted":false,"isSupressed":false,"isResolved":true,"scores":[]}]';
    ...
    test('Test Create New Marker Request - everything is ok', () => {
        const position = new MarkerPosition("/refactor/document", repo, 12);
        const marker = new PositionMarker("dsadsa", position, false);
        const user = "test-user";
        const score = 1;
        ...
        const response = converter.createNewMarkerRequest(marker, score, user);

        assert.strictEqual(response.content, "dsadsa");
        assert.strictEqual(response.user, user);
        assert.strictEqual(response.repository, repo);
        assert.strictEqual(response.score, 1);
        assert.strictEqual(response.line, 12);
    });

    test('Test Create New Marker Request - empty', () => {
        const converter = new ConverterService();
        const stats = undefined;

        const tableData = converter.formatToTableData(stats);
        
        assert.strictEqual("", tableData[0]);
        assert.strictEqual("", tableData[1]);
    });
});